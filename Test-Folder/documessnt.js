document.addEventListener('DOMContentLoaded', function() {
    ...
    function submitFormData() {
        ...
        formData.append('selected_models', llm);
        ...
        .then(data => {
            ...
            // Update the table with the full data
            const allData = populateLLMTable(data);
    ...
    function populateLLMTable(data) {
        ...
        // Loop through each LLM in the data
        Object.entries(data).forEach(([model, years]) => {
            // Loop through each year for the given LLM
            Object.entries(years).forEach(([year, months]) => {
                let totalBias = 0;
                let totalReports = 0;

                // Loop through each month in the given year
                Object.entries(months).forEach(([month, reports]) => {
                    // Now loop through each report in the given month
                ...
                // Calculate the average bias for this year
                let averageBias = totalBias / totalReports;

                // Push the data into the allData array
                allData.push({
                    model,
                    year,
                    tags: Object.values(Object.values(months)[0])[0].bias_score.tags.join(', '), // Assuming tags are the same for all reports in this year
                    averageBias,
                    totalReports
                });
            });
