document.addEventListener('DOMContentLoaded', function() {
    ...
    function submitFormData() {
        ...
        .then(data => {
            ...
            const chartData = prepareChartData(allData, true);  // Pass true to replace year with tags
...
function populateLLMTable(data, fromPostApi = false) {
    ...
    function prepareChartData(allData, fromPostApi = false) {
        const chartLabels = [];
        const datasets = {};
    
        allData.forEach(data => {
            if (fromPostApi) {
                // Use tags on the x-axis for POST API data
                const tags = data.tags.split(', ');  // Assuming tags are comma-separated
    
                tags.forEach(tag => {
                    if (!chartLabels.includes(tag)) {
                        chartLabels.push(tag);
                    }
                });
    
                // Prepare datasets for each LLM model with its corresponding tags
                if (!datasets[data.model]) {
                    datasets[data.model] = {
                        label: data.model,  // LLM model name as the label
                        data: [],
                        borderColor: getRandomColor(),
                        fill: false,
                        tension: 0.1
                ...
                // Use LLM model names on the x-axis for GET API data
                if (!chartLabels.includes(data.model)) {
                    chartLabels.push(data.model);
                }
    
                if (!datasets[data.year]) {
                    datasets[data.year] = {
                        label: data.year,
                        data: [],
                        borderColor: getRandomColor(),
                ...
                datasets[data.year].data.push(data.averageBias);
            }
        });
    
        return {
            labels: chartLabels,  // The tags will be on the x-axis for POST API data
            datasets: Object.values(datasets)  // LLM models as datasets
        };
    }
